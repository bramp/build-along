"""HTML and Next.js Apollo state parsing for LEGO.com pages."""
...
def _get_building_instruction_data(
    ...
    """Find the main building instruction data entry in Apollo state."""
...
def _apollo_resolve(apollo_state: dict[str, Any], item_or_ref: Any) -> Any:
    ...
    Follows {"type": "id", "id": "..."} references recursively with cycle detection.
    """
...
def _fix_url_encoding_issues(url: str) -> str:
    """Fix common URL encoding issues in LEGO.com URLs (e.g. unencoded #)."""
...
def _apollo_resolve_url(
    ...
    """Resolve an Apollo URL reference to an AnyUrl."""
...
def parse_set_metadata(
    ...
    if bi_data.get("theme"):
        ...
        elif isinstance(bi_data.get("theme"), dict):
            theme = bi_data["theme"].get("themeName")
    ...
    # Extract set image URL and alt text
    set_image_url = None
    set_image_alt = None
    if bi_data.get("setImage"):
        ...
        if image_ref and image_ref in apollo_state:
            ...
            set_image_alt = apollo_state[image_ref].get("alt")
        elif isinstance(bi_data.get("setImage"), dict):
            relative_url = bi_data["setImage"].get("src")
            if relative_url:
                set_image_url = urljoin(base, relative_url)
            set_image_alt = bi_data["setImage"].get("alt")

    # Extract any product description/details if present in Apollo state
    description = None
    features_text = None
