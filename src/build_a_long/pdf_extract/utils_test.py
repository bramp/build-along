...
class TestTransformForJson:
    ...
    def test_dict_without_tag_unchanged_order(self) -> None:
        """Test that dicts without __tag__ keep their key order."""
        data = {"x": 0, "y": 1, "z": 2}
        result = transform_for_json(data)
    ...
    def test_non_dict_values_unchanged(self) -> None:
        ...
        assert transform_for_json(42) == 42
        assert transform_for_json("hello") == "hello"
        assert transform_for_json([1, 2, 3]) == [1, 2, 3]
        assert transform_for_json(None) is None

    def test_combined_rounding_and_reordering(self) -> None:
        """Test that both rounding and reordering happen in one pass."""
        data = {
            "bbox": {"x": 1.234, "y": 5.678},
            "__tag__": "Test",
            "value": 3.14159,
        }
        result = transform_for_json(data)

        # Check __tag__ is first
        assert list(result.keys())[0] == "__tag__"

        # Check floats are rounded
        assert result["bbox"]["x"] == 1.23
        assert result["bbox"]["y"] == 5.68
