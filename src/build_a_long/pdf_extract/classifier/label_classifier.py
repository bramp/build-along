...
class LabelClassifier(BaseModel, ABC):
    ...
    def declare_constraints(
        ...
        """Declare CP-SAT constraints for this classifier's candidates.

        This method is called during constraint solver initialization to add
        constraints specific to this classifier. The default implementation does
        nothing - subclasses should override to add custom constraints.

        Example constraints:
        - Uniqueness: "At most one Step with step_number=4"
        - Hierarchy: "If Step selected, exactly one StepNumber child"
        - No orphans: "If Arrow selected, at least one Step must exist"
        - Spatial: "Diagrams must be assigned to closest Step" (post-solve)

        Note: Structural constraints (parent-child relationships based on Pydantic
        fields) are auto-generated by SchemaConstraintGenerator. Use this method
        for constraints that cannot be inferred from the schema.
