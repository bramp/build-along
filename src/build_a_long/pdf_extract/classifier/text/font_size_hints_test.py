"""Tests for font_size_hints module."""

import json

import pytest

from build_a_long.pdf_extract.classifier.text import FontSizeHints
from build_a_long.pdf_extract.cli.io import load_json
from build_a_long.pdf_extract.extractor import ExtractionResult, PageData
from build_a_long.pdf_extract.extractor.bbox import BBox
from build_a_long.pdf_extract.extractor.page_blocks import Text
from build_a_long.pdf_extract.fixtures import FIXTURES_DIR


def test_from_pages_with_all_sizes() -> None:
    """Test hint extraction when we have part counts and step numbers."""
    # Create mock pages with text blocks
    # Use multiple pages to simulate realistic instruction booklet
    page1 = PageData(
        page_number=1,
        bbox=BBox(0, 0, 100, 100),
        blocks=[
            Text(bbox=BBox(10, 10, 20, 20), text="2x", font_size=10.0, id=1),
            Text(bbox=BBox(10, 30, 20, 40), text="3x", font_size=10.0, id=2),
            Text(bbox=BBox(10, 50, 20, 60), text="4x", font_size=10.0, id=3),
            Text(bbox=BBox(10, 70, 20, 80), text="5x", font_size=10.0, id=4),
            Text(bbox=BBox(10, 80, 20, 90), text="6x", font_size=10.0, id=5),
            # Step numbers (1-999, not page ±1)
            Text(bbox=BBox(30, 10, 40, 20), text="99", font_size=16.0, id=6),
            Text(bbox=BBox(30, 30, 40, 40), text="100", font_size=16.0, id=7),
            Text(bbox=BBox(30, 50, 40, 60), text="101", font_size=16.0, id=8),
        ],
    )
    page2 = PageData(
        page_number=2,
        bbox=BBox(0, 0, 100, 100),
        blocks=[
            Text(bbox=BBox(10, 10, 20, 20), text="1x", font_size=12.0, id=1),
            Text(bbox=BBox(10, 30, 20, 40), text="2x", font_size=12.0, id=2),
            Text(bbox=BBox(10, 50, 20, 60), text="3x", font_size=12.0, id=3),
            # More step numbers
            Text(bbox=BBox(30, 10, 40, 20), text="50", font_size=14.0, id=4),
            Text(bbox=BBox(30, 30, 40, 40), text="51", font_size=14.0, id=5),
            Text(bbox=BBox(30, 50, 40, 60), text="52", font_size=14.0, id=6),
        ],
    )
    page3 = PageData(
        page_number=3,
        bbox=BBox(0, 0, 100, 100),
        blocks=[
            Text(bbox=BBox(10, 10, 20, 20), text="1x", font_size=14.0, id=1),
            Text(bbox=BBox(10, 30, 20, 40), text="2x", font_size=14.0, id=2),
            Text(bbox=BBox(10, 50, 20, 60), text="3x", font_size=14.0, id=3),
        ],
    )

    hints = FontSizeHints.from_pages([page1, page2, page3])

    assert hints.part_count_size == 10.0  # Most common (5 occurrences)
    # step_number_size comes from step_number_font_sizes (small integers 1-999)
    assert hints.step_number_size == 16.0  # Most common step number size (3 at 16.0)
    assert (
        hints.step_repeat_size == 14.0
    )  # 2nd most common step number size (3 at 14.0)

    # Remaining should be empty since all integers are accounted for
    assert hints.remaining_font_sizes == {}


def test_from_pages_with_two_sizes() -> None:
    """Test hint extraction when we only have part counts and some step numbers."""
    # Create multiple pages to meet MIN_SAMPLES requirement
    page1 = PageData(
        page_number=1,
        bbox=BBox(0, 0, 100, 100),
        blocks=[
            Text(bbox=BBox(10, 10, 20, 20), text="2x", font_size=10.0, id=1),
            Text(bbox=BBox(10, 30, 20, 40), text="3x", font_size=10.0, id=2),
            Text(bbox=BBox(10, 50, 20, 60), text="4x", font_size=10.0, id=3),
            # Step numbers (1-999, not page ±1)
            Text(bbox=BBox(30, 10, 40, 20), text="50", font_size=16.0, id=4),
            Text(bbox=BBox(30, 30, 40, 40), text="51", font_size=16.0, id=5),
            Text(bbox=BBox(30, 50, 40, 60), text="52", font_size=16.0, id=6),
        ],
    )
    page2 = PageData(
        page_number=2,
        bbox=BBox(0, 0, 100, 100),
        blocks=[
            Text(bbox=BBox(10, 10, 20, 20), text="1x", font_size=12.0, id=1),
            Text(bbox=BBox(10, 30, 20, 40), text="2x", font_size=12.0, id=2),
            Text(bbox=BBox(10, 50, 20, 60), text="3x", font_size=12.0, id=3),
        ],
    )
    page3 = PageData(
        page_number=3,
        bbox=BBox(0, 0, 100, 100),
        blocks=[],
    )

    hints = FontSizeHints.from_pages([page1, page2, page3])

    assert hints.part_count_size == 10.0
    assert hints.step_number_size == 16.0  # From step_number_font_sizes
    assert hints.step_repeat_size is None  # Not enough data for 2nd step number size

    # Remaining should be empty since all integers are step numbers
    assert hints.remaining_font_sizes == {}


def test_from_pages_with_one_size() -> None:
    """Test hint extraction when we only have 1 part count size."""
    # Create multiple pages to meet MIN_SAMPLES requirement
    page1 = PageData(
        page_number=1,
        bbox=BBox(0, 0, 100, 100),
        blocks=[
            Text(bbox=BBox(10, 10, 20, 20), text="2x", font_size=10.0, id=1),
            Text(bbox=BBox(10, 30, 20, 40), text="3x", font_size=10.0, id=2),
            Text(bbox=BBox(10, 50, 20, 60), text="4x", font_size=10.0, id=3),
        ],
    )
    page2 = PageData(
        page_number=2,
        bbox=BBox(0, 0, 100, 100),
        blocks=[],
    )
    page3 = PageData(
        page_number=3,
        bbox=BBox(0, 0, 100, 100),
        blocks=[],
    )

    hints = FontSizeHints.from_pages([page1, page2, page3])

    assert hints.part_count_size == 10.0
    assert hints.catalog_part_count_size is None
    assert hints.step_number_size is None

    # Remaining should be empty since the only size is known
    assert hints.remaining_font_sizes == {}


def test_from_pages_with_no_part_counts() -> None:
    """Test hint extraction when there are no part count patterns."""
    page1 = PageData(
        page_number=1,
        bbox=BBox(0, 0, 100, 100),
        blocks=[
            Text(
                bbox=BBox(10, 10, 20, 20), text="10", font_size=10.0, id=1
            ),  # Integer 1-999, not page ±1 -> step_number
            Text(
                bbox=BBox(10, 30, 20, 40), text="20", font_size=12.0, id=2
            ),  # Integer 1-999, not page ±1 -> step_number
        ],
    )

    hints = FontSizeHints.from_pages([page1])

    assert hints.part_count_size is None
    assert hints.catalog_part_count_size is None
    # step_number_size is None because we need MIN_SAMPLES (3) for confidence
    assert hints.step_number_size is None

    # remaining_font_sizes is empty since the integers went to step_number_font_sizes
    # (they just didn't meet MIN_SAMPLES threshold for step_number_size hint)
    assert hints.remaining_font_sizes == {}


def test_remaining_font_sizes_preserves_counts() -> None:
    """Test that remaining font sizes preserve their original counts."""
    # Need multiple pages to ensure data goes to instruction section
    page1 = PageData(
        page_number=1,
        bbox=BBox(0, 0, 100, 100),
        blocks=[
            Text(bbox=BBox(10, 10, 20, 20), text="2x", font_size=10.0, id=1),
            Text(bbox=BBox(10, 30, 20, 40), text="3x", font_size=12.0, id=2),
            # Integers 1-999 (not page ±1) go to step_number_font_sizes
            Text(bbox=BBox(10, 50, 20, 60), text="50", font_size=14.0, id=3),
            Text(bbox=BBox(10, 70, 20, 80), text="60", font_size=14.0, id=4),
            Text(bbox=BBox(30, 10, 40, 20), text="70", font_size=16.0, id=5),
            # Large integers (>999) go to remaining_font_sizes
            Text(bbox=BBox(30, 30, 40, 40), text="1000", font_size=18.0, id=6),
            Text(bbox=BBox(30, 50, 40, 60), text="2000", font_size=18.0, id=7),
            Text(bbox=BBox(30, 70, 40, 80), text="3000", font_size=20.0, id=8),
        ],
    )
    page2 = PageData(
        page_number=2,
        bbox=BBox(0, 0, 100, 100),
        blocks=[],
    )
    page3 = PageData(
        page_number=3,
        bbox=BBox(0, 0, 100, 100),
        blocks=[],
    )

    hints = FontSizeHints.from_pages([page1, page2, page3])

    # remaining_font_sizes only has large integers (>999, not element IDs)
    assert hints.remaining_font_sizes == {"18.0": 2, "20.0": 1}


def test_catalog_section_separation() -> None:
    """Test that catalog pages are analyzed separately from instruction pages."""
    # Create instruction pages without catalog element IDs
    instruction_pages = [
        PageData(
            page_number=i,
            bbox=BBox(0, 0, 100, 100),
            blocks=[
                Text(bbox=BBox(10, 10, 20, 20), text="1x", font_size=12.0, id=1),
                Text(bbox=BBox(10, 30, 20, 40), text="2x", font_size=12.0, id=2),
                Text(bbox=BBox(10, 50, 20, 60), text="3x", font_size=12.0, id=3),
            ],
        )
        for i in range(1, 7)
    ]

    # Catalog pages with >3 catalog element IDs (6-7 digit numbers)
    # and smaller font for part counts
    catalog_pages = [
        PageData(
            page_number=i,
            bbox=BBox(0, 0, 100, 100),
            blocks=[
                # Add 4 catalog element IDs to mark this as a catalog page
                Text(bbox=BBox(5, 5, 10, 10), text="6055739", font_size=6.0, id=100),
                Text(bbox=BBox(5, 15, 10, 20), text="6055740", font_size=6.0, id=101),
                Text(bbox=BBox(5, 25, 10, 30), text="6055741", font_size=6.0, id=102),
                Text(bbox=BBox(5, 35, 10, 40), text="6055742", font_size=6.0, id=103),
                # Part counts on catalog pages
                *[
                    Text(
                        bbox=BBox(10, j * 10, 20, j * 10 + 10),
                        text=f"{j}x",
                        font_size=8.0,
                        id=j,
                    )
                    for j in range(1, 21)  # 20 part counts per catalog page
                ],
            ],
        )
        for i in range(7, 10)
    ]

    all_pages = instruction_pages + catalog_pages
    hints = FontSizeHints.from_pages(all_pages)

    # Despite catalog having more total occurrences (60 vs 18),
    # part_count_size should come from instruction pages
    assert hints.part_count_size == 12.0
    # Catalog part count should be the smaller size from catalog section
    assert hints.catalog_part_count_size == 8.0


def test_catalog_size_validation() -> None:
    """Test that catalog sizes larger than instruction sizes generate a warning."""
    # Instruction pages with smaller font
    instruction_pages = [
        PageData(
            page_number=i,
            bbox=BBox(0, 0, 100, 100),
            blocks=[
                Text(bbox=BBox(10, 10, 20, 20), text="1x", font_size=8.0, id=1),
                Text(bbox=BBox(10, 30, 20, 40), text="2x", font_size=8.0, id=2),
                Text(bbox=BBox(10, 50, 20, 60), text="3x", font_size=8.0, id=3),
            ],
        )
        for i in range(1, 7)
    ]

    # Catalog pages with >3 catalog element IDs and LARGER font
    # (unusual, but we still keep the value and just warn)
    catalog_pages = [
        PageData(
            page_number=i,
            bbox=BBox(0, 0, 100, 100),
            blocks=[
                # Add 4 catalog element IDs to mark this as a catalog page
                Text(bbox=BBox(5, 5, 10, 10), text="6055739", font_size=6.0, id=100),
                Text(bbox=BBox(5, 15, 10, 20), text="6055740", font_size=6.0, id=101),
                Text(bbox=BBox(5, 25, 10, 30), text="6055741", font_size=6.0, id=102),
                Text(bbox=BBox(5, 35, 10, 40), text="6055742", font_size=6.0, id=103),
                # Part counts with larger font
                *[
                    Text(
                        bbox=BBox(10, j * 10, 20, j * 10 + 10),
                        text=f"{j}x",
                        font_size=12.0,
                        id=j,
                    )
                    for j in range(1, 11)
                ],
            ],
        )
        for i in range(7, 10)
    ]

    all_pages = instruction_pages + catalog_pages
    hints = FontSizeHints.from_pages(all_pages)

    assert hints.part_count_size == 8.0
    # Catalog size is kept even though 12.0 > 8.0 (just generates a warning)
    assert hints.catalog_part_count_size == 12.0


class TestFontSizeHintsGolden:
    """Golden file tests for FontSizeHints.from_pages."""

    @pytest.mark.parametrize(
        "fixture_file",
        [
            "6055741_raw.json.bz2",
            "6509377_raw.json.bz2",
            "6580053_raw.json.bz2",
        ],
    )
    def test_from_pages_matches_golden(self, fixture_file: str) -> None:
        """Test that FontSizeHints.from_pages matches the golden file.

        This test:
        1. Loads a raw page fixture (compressed)
        2. Runs FontSizeHints.from_pages
        3. Compares against golden file

        To generate golden files, run:
            pants run \\
                src/build_a_long/pdf_extract/classifier/ \\
                tools:generate-font-hints-golden
        """
        fixture_path = FIXTURES_DIR / fixture_file

        # Determine golden file path
        golden_file = fixture_file.replace("_raw.json.bz2", "_font_hints_expected.json")
        golden_path = FIXTURES_DIR / golden_file

        # Load the input fixture
        json_data = load_json(fixture_path)
        extraction: ExtractionResult = ExtractionResult.model_validate_json(
            json.dumps(json_data)
        )  # type: ignore[assignment]

        # Run FontSizeHints.from_pages
        hints = FontSizeHints.from_pages(extraction.pages)

        # Serialize the hints to dict using built-in to_dict()
        actual = hints.model_dump()

        # Check if golden file exists
        if not golden_path.exists():
            pytest.skip(
                f"Golden file not found: {golden_file}\n"
                "Run: pants run "
                "src/build_a_long/pdf_extract/classifier/"
                "tools:generate-font-hints-golden"
            )

        # Load expected results
        with open(golden_path) as f:
            expected = json.load(f)

        # Compare results
        assert actual == expected, (
            f"FontSizeHints mismatch for {fixture_file}.\n"
            f"Expected: {json.dumps(expected, indent=2, sort_keys=True)}\n"
            f"Actual: {json.dumps(actual, indent=2, sort_keys=True)}"
        )
