...
class PartsListConfig(BaseModel):
    ...
    min_score: Weight = Field(
        default=0.5, description="Minimum score threshold for parts list candidates."
    )
    ...
    max_area_ratio: float = Field(
        ...
        le=1.0,
        description=(
            "Maximum ratio of page area a parts list can occupy (0.0-1.0). Drawings "
            "larger than this fraction of the page are rejected as they're likely the "
            "entire page background rather than actual parts lists. Default is 0.75 "
            "(75% of page area)."
        ),
    )
